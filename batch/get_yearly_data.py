...
def get_fnguide_table(tickers):
    ...
    for code, name in tickers.items():  # 각 종목 코드와 기업명을 순회
        ...
        url = requests.get(f'http://comp.fnguide.com/SVO2/ASP/SVD_main.asp?pGB=1&gicode=A{code}')
        ...
        tr_2 = thead.find('tr', {'class': 'td_gapcolor2'}).find_all('th')  # 헤더 행의 모든 열 가져오기
        year_list = []  # 연도 리스트 초기화

        for th in tr_2:  # 헤더의 각 열을 순회
            try:
                temp_year = th.find('span', {'class': 'txt_acd'}).text  # 연도 텍스트 가져오기
            except:
...
final_df = get_fnguide_table(tickers)  # 데이터 크롤링 및 데이터프레임 생성
